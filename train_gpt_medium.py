...
except Exception:  # pragma: no cover - wandb may be unavailable in some environments
    wandb = None  # noqa: F401
...
@torch.compile
def update(acc_bf16_view_u16: Tensor, mantissa: Tensor, momentum_buffer: Tensor, grad: Tensor, momentum: Tensor,
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, block_mask: BlockMask, lambdas: Tensor):
        ...
        q, k, v = F.linear(x, self.qkvo_w[:3].flatten(end_dim=1)).view(B, T, 3 * self.num_heads, self.head_dim).chunk(3,
                                                                                                                      dim=-2)
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, x0: Tensor, block_mask: BlockMask, lambdas: Tensor,
                sa_lambdas: Tensor):
...
class Hyperparameters:
    # data
    train_files = "data/dclm_baseline/dclm_baseline_train_*.bin"  # input .bin to train on
    val_files = "data/dclm_baseline/dclm_baseline_val_*.bin"  # input .bin to eval validation loss on
...
def _wandb_init_if_available():
    """Initialize wandb on the master process if available and not disabled."""
    if not master_process:
        return None
    if wandb is None:
        return None
    # honor common disable flags
    if os.environ.get("WANDB_DISABLED", "").lower() in ("1", "true", "yes"):
        return None
    project = os.environ.get("WANDB_PROJECT", "modded-nanogpt-medium")
    entity = os.environ.get("WANDB_ENTITY")  # optional
    init_kwargs = dict(project=project, name=run_id_full, config={})
    if entity:
...
def wb_log(metrics: dict, *, step: int | None = None):
    """Safely log to wandb from the master process if initialized."""
    try:
        if master_process and wandb is not None and getattr(wandb, "run", None) is not None:
            if step is None:
                wandb.log(metrics)
            else:
...
# Initialize wandb (if available) after environment info is printed
_wandb_init_if_available()
if master_process and wandb is not None and getattr(wandb, "run", None) is not None:
    try:
        # Log static run metadata and hyperparameters
        wandb.config.update({
...
last_val_loss: torch.Tensor | None = None
for step in range(train_steps + 1):
    ...
    if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
        ...
        print0(
            ...
            console=True)
        last_val_loss = val_loss.detach()
        # wandb: log validation loss and cumulative tokens (no new tokens processed yet on this step)
        wb_log({
            "val/loss": float(val_loss.item()),
            "cumulative_tokens": int(tokens_per_step * step),
        }, step=step)
        model.train()
    ...
    # detach a copy for logging before backward
    train_loss = loss.detach()
    # average the loss across processes for a global view
...
# Final wandb logging and finish
if master_process and wandb is not None and getattr(wandb, "run", None) is not None:
    try:
        final_metrics = {
            "tokens": int(tokens_per_step * train_steps),
        }
        if last_val_loss is not None:
            final_metrics["final_val_loss"] = float(last_val_loss.item())
        wb_log(final_metrics, step=train_steps)
        wandb.finish()
    except Exception:
        pass
