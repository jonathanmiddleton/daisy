...
class FakeDataGenerator(DataGeneratorProtocol):
    """
    ...
    def __init__(self, *, sequence_length: int, world_size: int, rank: int):
        assert sequence_length % world_size == 0
        self.batch_size = int(sequence_length)        # GLOBAL batch size (matches real generator API)
    ...
    def __next__(self):
        ...
        inputs = torch.full((self.sequence_length,), base, dtype=torch.int32)
...
@pytest.mark.parametrize("global_val_tokens", [32])
@pytest.mark.parametrize("sequence_length", [4, 8])
def test_eval_consumes_total_tokens_globally_across_world_sizes(world_size, global_val_tokens, sequence_length):
    steps_per_rank = global_val_tokens // world_size // sequence_length
    ...
    for rank in range(world_size):
        ddg = FakeDataGenerator(sequence_length=sequence_length, world_size=world_size, rank=rank)
        evaluator = Evaluator(
            data_generator=ddg,
            distributed_enabled=False,  # not actually using torch.distributed in this unit test
            global_val_tokens=global_val_tokens,
            rank=rank,
        ...
        result = evaluator.eval(model=model, schedule=1.0)
        ...
        assert ddg.calls == steps_per_rank
        per_rank_calls.append(ddg.calls)

        # Each rank processes local_batch_size tokens per step
        expected_local_tokens = sequence_length * steps_per_rank
    ...
    assert global_tokens_processed == global_val_tokens

    # Sanity: per-rank calls are all equal to steps
    assert all(c == steps_per_rank for c in per_rank_calls)
...
def test_eval_uses_data_generator_batches_exactly():
    ...
    sequence_length = 8
    steps = 5
    global_val_tokens = world_size * steps * sequence_length
    ...
    for rank in range(world_size):
        ...
        evaluator.eval(model=model, schedule=1.0)
        ...
        for s, (inputs, targets) in enumerate(model.seen_batches):
            assert inputs.shape == (sequence_length,)
            assert targets.shape == (sequence_length,)
